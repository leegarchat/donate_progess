...
public final class Magnifier {
    ...
    // =========================================================================================
    // [CUSTOM INJECTION START] - Pixel Extra Parts: Magnifier Customization
    // =========================================================================================

    // Settings Keys (Pine — hardcoded suffix for system/framework build)
    private static final String KEY_ENABLED   = "magnifier_custom_enabled_pine";
    private static final String KEY_ZOOM      = "magnifier_custom_zoom_pine";
    private static final String KEY_SIZE      = "magnifier_custom_size_pine";
    private static final String KEY_SHAPE     = "magnifier_custom_shape_pine";
    private static final String KEY_OFFSET_Y  = "magnifier_custom_offset_y_pine";
    ...
    private static int applyCustomParams(@NonNull Builder params) {
        ...
        try {
            ...
            float sizeScale = Settings.Global.getFloat(
                    ctx.getContentResolver(), KEY_SIZE, DEFAULT_SIZE);
            int shape = Settings.Global.getInt(
                    ctx.getContentResolver(), KEY_SHAPE, SHAPE_DEFAULT);
            int offsetYDp = Settings.Global.getInt(
                    ctx.getContentResolver(), KEY_OFFSET_Y, 0);

            // --- Zoom override ---
            if (customZoom > 0f) {
                params.mZoom = customZoom;
            }

            // --- Size scale: enlarge window dimensions ---
            if (Math.abs(sizeScale - 1.0f) > 0.01f && sizeScale > 0f) {
                params.mWidth = Math.max(1, Math.round(params.mWidth * sizeScale));
                params.mHeight = Math.max(1, Math.round(params.mHeight * sizeScale));
            }

            // --- Shape: square or circle ---
            if (shape == SHAPE_SQUARE || shape == SHAPE_CIRCLE) {
                int side = Math.max(params.mWidth, params.mHeight);
                params.mWidth = side;
                params.mHeight = side;

                if (shape == SHAPE_CIRCLE) {
                    params.mCornerRadius = side / 2.0f;
                }
            }

            // --- Vertical offset (dp → px) ---
            if (offsetYDp != 0) {
            ...
            if (params.mWidth > 0 && params.mHeight > 0 && params.mZoom > 0f) {
                params.mSourceWidth = params.mIsFishEyeStyle
                        ? params.mWidth : Math.round(params.mWidth / params.mZoom);
                params.mSourceHeight = Math.round(params.mHeight / params.mZoom);
            }
        } catch (Exception e) {
    ...
    /**
     * Overrides zoom in setZoom() if custom magnifier is enabled.
     * Returns the (possibly overridden) zoom value.
     */
    private float customOverrideZoom(float requestedZoom) {
        try {
            Context ctx = mView.getContext();
            if (ctx == null) return requestedZoom;
            int enabled = Settings.Global.getInt(
                    ctx.getContentResolver(), KEY_ENABLED, 0);
            if (enabled != 1) return requestedZoom;
            float customZoom = Settings.Global.getFloat(
                    ctx.getContentResolver(), KEY_ZOOM, DEFAULT_ZOOM);
            if (customZoom > 0f && Math.abs(customZoom - requestedZoom) > 0.01f) {
                return customZoom;
            }
        } catch (Exception e) {
            Log.e(TAG, "[PixelParts] Magnifier customOverrideZoom failed", e);
        }
    ...
    public void setZoom(@FloatRange(from = 0f) float zoom) {
        // [CUSTOM INJECTION] Override zoom if custom magnifier is enabled
        zoom = customOverrideZoom(zoom);
